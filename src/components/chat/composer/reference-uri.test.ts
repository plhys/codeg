...
describe("parseCodegReferenceUri", () => {
  it("returns null for non-reference schemes", () => {
    ...
    expect(parseCodegReferenceUri("veryagent://unknown/1", "x")).toBeNull()
  })
  ...
  it("parses an agent uri, stripping a leading @ from the label", () => {
    expect(
      parseCodegReferenceUri("veryagent://agent/codex", "@Codex")
    ).toMatchObject({
      refType: "agent",
      id: "codex",
      label: "Codex",
  ...
  it("falls back to the agent type when the agent label is empty", () => {
    expect(
      parseCodegReferenceUri("veryagent://agent/claude_code", "")
    ).toMatchObject({
      refType: "agent",
      id: "claude_code",
      label: "claude_code",
  ...
  it("parses a new-format session uri, recovering the agent type", () => {
    expect(
      parseCodegReferenceUri("veryagent://session/codex_abc123", "My chat")
    ).toMatchObject({
      refType: "session",
      id: "codex_abc123",
      label: "My chat",
  ...
  it("never splits an agent type on its first underscore", () => {
    ...
    expect(
      parseCodegReferenceUri("veryagent://session/claude_code_sess-9", "")
    ).toMatchObject({
      id: "claude_code_sess-9",
      meta: { agentType: "claude_code" },
    })
    expect(
      parseCodegReferenceUri("veryagent://session/open_code_x", "")?.meta
    ).toEqual({ agentType: "open_code" })
    expect(
      parseCodegReferenceUri("veryagent://session/open_claw_y", "")?.meta
    ).toEqual({ agentType: "open_claw" })
  })

  it("treats a legacy numeric session id as opaque (no agent icon)", () => {
    expect(
      parseCodegReferenceUri("veryagent://session/123", "Login")
    ).toMatchObject({
      refType: "session",
      id: "123",
      label: "Login",
  ...
  it("treats a non-agent-prefixed token as a plain session id", () => {
    expect(
      parseCodegReferenceUri("veryagent://session/randomtoken", "")
    ).toMatchObject({ refType: "session", id: "randomtoken", meta: null })
  })

  it("falls back to #id for an empty session label", () => {
    expect(parseCodegReferenceUri("veryagent://session/123", "")?.label).toBe(
      "#123"
    )
  })
  ...
  it("parses a commit uri, deriving the short hash", () => {
    expect(
      parseCodegReferenceUri("veryagent://commit/%2Frepo@abc1234def5678", "abc1234")
    ).toMatchObject({
      refType: "commit",
      id: "abc1234def5678",
      label: "abc1234",
  ...
  it("parses a skill uri, keeping the literal `/`·`$` token as the label", () => {
    expect(
      parseCodegReferenceUri("veryagent://skill/review", "/review")
    ).toMatchObject({
      refType: "skill",
      id: "review",
      label: "/review",
  ...
  it("falls back to a /-prefixed id for an empty skill label", () => {
    expect(parseCodegReferenceUri("veryagent://skill/deploy", "")?.label).toBe(
      "/deploy"
    )
  })
  ...
  it("parses an embedded-attachment uri as an inert file badge", () => {
    expect(
      parseCodegReferenceUri("veryagent://embedded/9f3c-uuid", "report.pdf")
    ).toMatchObject({
      refType: "file",
      label: "report.pdf",
  ...
  it("falls back to a generic label for an empty embedded-attachment label", () => {
    expect(
      parseCodegReferenceUri("veryagent://embedded/9f3c-uuid", "")?.label
    ).toBe("resource")
  })
