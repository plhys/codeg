...
`veryagent-mcp` must sit next to its parent binary at runtime — installers, the Docker image, and the Tauri sidecar bundler all place it next to `veryagent` / `veryagent-server`. Source builds and custom layouts can override the lookup with the `CODEG_MCP_BIN=/abs/path/veryagent-mcp` env var. If the companion is missing, delegation is skipped (a single warning is logged) and the rest of the agent session keeps working.
...
> Tip: when you have a fresh `veryagent-mcp` build under `src-tauri/target/release/` and want to point a manually-launched `veryagent-server` at it without reinstalling, export `CODEG_MCP_BIN=$(pwd)/src-tauri/target/release/veryagent-mcp`.
...
Pre-built binaries (with bundled web assets) are available on the [Releases](https://github.com/xintaofei/veryagent/releases) page:
...
CODEG_STATIC_DIR=../out ./target/release/veryagent-server          # veryagent-mcp is picked up as a sibling
```

If you keep the two binaries in separate directories, set `CODEG_MCP_BIN=/abs/path/to/veryagent-mcp` so the runtime can still find the companion; without it, multi-agent delegation is silently disabled.
...
```bash
CODEG_STATIC_DIR=./web ./veryagent-server --supervise
```
...
| `CODEG_DATA_DIR`               | `~/.local/share/veryagent` | SQLite database directory (also roots `uploads/`, `pets/`)                                                                                                                                                                                                                                                                                                                                                                       |
| `CODEG_STATIC_DIR`             | `./web` or `./out`     | Next.js static export directory                                                                                                                                                                                                                                                                                                                                                                                                  |
| `CODEG_MCP_BIN`                | _(unset)_              | Absolute path to the `veryagent-mcp` companion. Overrides the default sibling-of-executable + `PATH` lookup. Use this for source builds or custom layouts where the companion lives outside the server's install directory.                                                                                                                                                                                                          |
| `CODEG_SKIP_SIDECAR`           | _(unset)_              | Frontend-only convenience for `pnpm tauri dev` / `pnpm tauri build` — when `1`, skips building the `veryagent-mcp` sidecar. Delegation is disabled in that build; ship-quality artifacts must leave it unset.                                                                                                                                                                                                                        |
| `CODEG_UPLOAD_MAX_TOTAL_BYTES` | _(unset)_              | Hard cap on total bytes resident under `<data dir>/uploads/`. Plain decimal byte count (e.g. `10737418240` for 10 GiB). Unset, `0`, or an unparseable value disables the cap and prints a startup line so the posture is visible. The cap is enforced within a single `veryagent-server` process — horizontally-scaled deployments sharing one `uploads/` volume need external coordination (file lock, Redis, reverse-proxy quota). |
