...
`veryagent-mcp` 在运行时必须与其父二进制位于同一目录——安装器、Docker 镜像和 Tauri sidecar 打包器都会把它放在 `veryagent` / `veryagent-server` 旁边。源码构建和自定义部署可以通过 `CODEG_MCP_BIN=/abs/path/veryagent-mcp` 环境变量覆盖查找路径。如果协作进程缺失，委托功能会被跳过（仅记录一条警告日志），其余 agent 会话仍可正常工作。
...
> 提示：当你在 `src-tauri/target/release/` 下有新构建的 `veryagent-mcp` 并想让手动启动的 `veryagent-server` 在不重新安装的情况下指向它时，可以导出 `CODEG_MCP_BIN=$(pwd)/src-tauri/target/release/veryagent-mcp`。
...
预构建二进制文件（已打包 Web 前端资源）可在 [Releases](https://github.com/xintaofei/veryagent/releases) 页面下载：
...
CODEG_STATIC_DIR=../out ./target/release/veryagent-server          # veryagent-mcp 会作为同级二进制被自动发现
```

> 如果两个二进制分别存放在不同目录，请设置 `CODEG_MCP_BIN=/abs/path/to/veryagent-mcp`，运行时才能找到协作进程；否则多智能体委托会被静默禁用。
...
| `CODEG_DATA_DIR`               | `~/.local/share/veryagent` | SQLite 数据库目录（同时也是 `uploads/`、`pets/` 的根目录）                                                                                                                                                                                                                                                  |
| `CODEG_STATIC_DIR`             | `./web` 或 `./out`     | Next.js 静态导出目录                                                                                                                                                                                                                                                                                        |
| `CODEG_MCP_BIN`                | _（未设置）_           | `veryagent-mcp` 协作进程的绝对路径。会覆盖默认的"可执行文件同级目录 + `PATH`"查找逻辑。用于源码构建或协作进程不在服务端安装目录内的自定义部署。                                                                                                                                                                  |
| `CODEG_SKIP_SIDECAR`           | _（未设置）_           | 仅供 `pnpm tauri dev` / `pnpm tauri build` 调试前端时使用——当值为 `1` 时，跳过 `veryagent-mcp` sidecar 的构建。此类构建不支持委托功能；发布质量的产物必须保持此变量未设置。                                                                                                                                      |
| `CODEG_UPLOAD_MAX_TOTAL_BYTES` | _（未设置）_           | `<data dir>/uploads/` 下所有文件总字节数的硬上限。十进制字节数（例如 `10737418240` 表示 10 GiB）。未设置、`0` 或无法解析的值会禁用上限，并在启动时打印一行日志以便观察当前状态。该上限仅在单个 `veryagent-server` 进程内生效——共享一个 `uploads/` 卷的横向扩展部署需要外部协调（文件锁、Redis、反向代理配额）。 |
