...
`veryagent-mcp` 在執行階段必須與其父二進位位於同一目錄——安裝程式、Docker 映像和 Tauri sidecar 打包工具都會將它放在 `veryagent` / `veryagent-server` 旁邊。原始碼建置和自訂部署可以透過 `CODEG_MCP_BIN=/abs/path/veryagent-mcp` 環境變數覆寫查詢路徑。若協作行程缺失，委派功能會被略過（僅記錄一則警告日誌），其餘 agent 會話仍可正常運作。
...
> 提示：當你在 `src-tauri/target/release/` 下有新建置的 `veryagent-mcp` 並想讓手動啟動的 `veryagent-server` 在不重新安裝的情況下指向它時，可以匯出 `CODEG_MCP_BIN=$(pwd)/src-tauri/target/release/veryagent-mcp`。
...
預建置二進位檔（已打包 Web 前端資源）可在 [Releases](https://github.com/xintaofei/veryagent/releases) 頁面下載：
...
CODEG_STATIC_DIR=../out ./target/release/veryagent-server          # veryagent-mcp 會作為同級二進位被自動探測
```

> 若兩個二進位分別存放在不同目錄，請設定 `CODEG_MCP_BIN=/abs/path/to/veryagent-mcp`，執行階段才能找到協作行程；否則多智慧體委派會被靜默停用。
...
| `CODEG_DATA_DIR`               | `~/.local/share/veryagent` | SQLite 資料庫目錄（同時也是 `uploads/`、`pets/` 的根目錄）                                                                                                                                                                                                                                                        |
| `CODEG_STATIC_DIR`             | `./web` 或 `./out`     | Next.js 靜態匯出目錄                                                                                                                                                                                                                                                                                              |
| `CODEG_MCP_BIN`                | _（未設定）_           | `veryagent-mcp` 協作行程的絕對路徑。會覆寫預設的「可執行檔同級目錄 + `PATH`」查詢邏輯。用於原始碼建置或協作行程不在伺服器安裝目錄內的自訂部署。                                                                                                                                                                       |
| `CODEG_SKIP_SIDECAR`           | _（未設定）_           | 僅供 `pnpm tauri dev` / `pnpm tauri build` 調試前端時使用——當值為 `1` 時，略過 `veryagent-mcp` sidecar 的建置。此類建置不支援委派功能；發行品質的產出物必須保持此變數未設定。                                                                                                                                          |
| `CODEG_UPLOAD_MAX_TOTAL_BYTES` | _（未設定）_           | `<data dir>/uploads/` 下所有檔案總位元組數的硬上限。十進位位元組數（例如 `10737418240` 表示 10 GiB）。未設定、`0` 或無法解析的值會停用上限，並在啟動時印出一行日誌以便觀察當前狀態。該上限僅在單一 `veryagent-server` 行程內生效——共用同一個 `uploads/` 卷的橫向擴展部署需要外部協調（檔案鎖、Redis、反向代理配額）。 |
